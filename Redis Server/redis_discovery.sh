...
if [ "$DISCOVERY_TYPE" != "general" ] && [ "$DISCOVERY_TYPE" != "stats" ] && [ "$DISCOVERY_TYPE" != "replication" ] && [ "$DISCOVERY_TYPE" != "databases" ]; then
	...
	echo "databases - generates report for avalable databases"
...
discover_redis_available_databases() {
    HOST=$1
    PORT=$2
    PASSWORD=$3
    ...
    if [[ $ALIVE != "PONG" ]]; then
        ...
        DATABASES=$($REDIS_CLI -h $HOST -p $PORT -a "$PASSWORD" info keyspace | grep db | awk -F":" '{print $1}')
    fi
...
# GENERATE ZABBIX DISCOVERY JSON REPONSE #
generate_databases_discovery_json() {
    HOST=$1
    PORT=$2
    DATABASE=$3
    INSTANCE=$4
...
for s in $LIST; do
    ...
    if [[ ${#PASSWORDS[@]} -ne 0 ]]; then
        ...
        do
			...
			DATABASES=$(discover_redis_available_databases $HOST $PORT $PASSWORD)
            ...
            if [[ -n $INSTANCE ]]; then
                ...
                elif [[ $DISCOVERY_TYPE == "databases" ]]; then
                    for DATABASE in ${DATABASES}; do
                        generate_databases_discovery_json $HOST $PORT $DATABASE $INSTANCE
                    done
		...
		DATABASES=$(discover_redis_available_databases $HOST $PORT "")
